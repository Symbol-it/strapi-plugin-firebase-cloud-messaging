...
const deleteKey = key => {
  return strapi.query('core_store').delete({ key: `plugin_notifications_configuration_${key}` });
};
...
const moveKey = (oldKey, newKey) => {
  return strapi.query('core_store').update(
    {
      key: `plugin_notifications_configuration_${oldKey}`,
    },
    {
      key: `plugin_notifications_configuration_${newKey}`,
    }
  );
};

const getAllConfigurations = () => findByKey('plugin_notifications_configuration');
